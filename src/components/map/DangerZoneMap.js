  ...
  if (error) {
    return (
      <div className="h-96 bg-gray-100 rounded-lg relative">
        {/* Show map in background even with error */}
        <MapContainer 
        ...
        >
          ...
          {/* Render danger zones with default location */}
          {dangerZones.map(zone => {
            const lat = defaultCenter[0] + (Math.random() * 0.02 - 0.01);
            const lng = defaultCenter[1] + (Math.random() * 0.02 - 0.01);
            return (
              <React.Fragment key={zone.id}>
                {renderDangerZones({...zone, lat, lng})}
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-20">
          <div className="bg-white rounded-lg p-6 max-w-sm mx-4 text-center shadow-lg">
            <svg className="h-12 w-12 text-yellow-500 mx-auto mb-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <h4 className="text-lg font-medium text-gray-900 mb-2">Location Access Limited</h4>
